<?php
    ...
    $prep_stmt = "SELECT * FROM ".$t_nona_rhn." WHERE TROUBLE_NO = ? LIMIT 1";
                    ...
                    if ($stmt) {
                        ...
                        $stmt->execute();
                        $stmt->bind_result($tiket, $ind, $inama, $iperihal, $imedia, $ikomplain, $icp, $istatus, $iketerangan, $itanggal, $iflag);
                        $stmt->fetch();
                        $stmt->close();
                        
                        if (!empty($itiket)) {
                            // A user with this email address already exists
                            $title = "EDIT ORDER RIHANA";
                        }
                        else {
                            $title = "INSERT ORDER RIHANA";
                            $itiket = $itiketalt;
                        }
                        if(!empty($itanggal)){
...
?>
                    <form class="uk-form sd-box" action="" method="POST" id="modalform" name="registration-form">
                            ...
                            <div class="uk-grid uk-grid-small" data-uk-grid-margin="">
                                ...
                                <div class="uk-form-icon uk-width-small-1-1">
                                        ...
                                        <input type='text' placeholder='PERIHAL' class='uk-width-1-1' name="perihal" value="<?php echo $iperihal; ?>">
                                </div>
                                <div class="uk-form-icon uk-width-medium-1-2 uk-width-small-1-1">
                                        <select class="uk-width-1-1" name="media">
                                            <option value='0' <?php if($imedia == '') echo 'selected'; ?>># MEDIA</option>
                                            <option value='1' <?php if($imedia == 'Email') echo 'selected'; ?>>Email</option>
                                            <option value='2' <?php if($imedia == 'Twitter') echo 'selected'; ?>>Twitter</option>
                                            <option value='3' <?php if($imedia == 'Facebook') echo 'selected'; ?>>Facebook</option>
                                        </select>
                                </div>
                                <div class="uk-form-icon uk-width-medium-1-2 uk-width-small-1-1">
                                        <select class="uk-width-1-1" name="status">
                                            <option value='0' <?php if($istatus == '') echo 'selected'; ?>># STATUS</option>
                                            <option value='1' <?php if($istatus == 'In Progress') echo 'selected'; ?>>IN PROGRESS</option>
                                            <option value='2' <?php if($istatus == 'Not Handled') echo 'selected'; ?>>NOT HANDLED</option>
                                        </select>
                                </div>
                                <div class="uk-form-icon uk-width-small-1-1" >
                                        <textarea class='uk-width-1-1' placeholder="KOMPLAIN (max. 200 digit)" name="komplain"><?php echo $ikomplain; ?></textarea>
                                </div>
                                <div class="uk-form-icon uk-width-small-1-1" >
                                ...
                                <div class="uk-form-icon uk-width-medium-6-10 uk-width-small-1-1 uk-clearfix">
                                        ...
                                        <button type="submit" class="uk-button uk-button-primary uk-float-right" value="1" name="submit" onclick="return formcheck(
                                           ...
                                           this.form.nd,
                                           this.form.nama,
                                           this.form.perihal);">SUBMIT</button>
                                </div>
                            </div>
                        </div>
                    </form>
    ...
    // Check each field has a value
        if (tiket.value == '' || nd.value == '' || nama.value == '' || perihal.value == '') {
