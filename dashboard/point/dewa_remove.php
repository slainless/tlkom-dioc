<?php
    ...
    if (isset($_POST['tiket'], $_POST["remove"])) {
        ...
        if(empty($message) && $remove == 1):
            $prep_stmt = "SELECT TROUBLE_NO FROM ".$t_nona_odw." WHERE TROUBLE_NO = ? LIMIT 1";
        ...
        if(empty($message) && $remove == 1){
            ...
            if($_SESSION["level"] < 3):
                $prep_stmt = "DELETE FROM ".$t_nona_odw." WHERE TROUBLE_NO = ?";
                $stmt = $query->prepare($prep_stmt);
                ...
                if(empty($message)):
                    $prep_stmt = "UPDATE ".$t_nona." SET ODW_F = 0 WHERE TROUBLE_NO = ?";
                    $stmt = $query->prepare($prep_stmt);
                ...
                if(empty($message)):
                    $prep_stmt = "UPDATE ".$t_point." SET ODW_F = 0 WHERE TROUBLE_NO = ?";
                    $stmt = $query->prepare($prep_stmt);
