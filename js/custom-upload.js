 ...
 * http://hayageek.com/
 * Customized for UIKit by Stardusk
 */
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            dragDropContainerClass: "uk-placeholder uk-text-center",
            dragDropHoverClass: "state-hover",
            errorClass: "uk-alert uk-alert-danger",
            uploadButtonClass: "uk-form-file",
            dragDropStr: "<span> Drop the file or </span>",
            uploadStr:"Select it",
            abortStr: "Abort",
            cancelStr: "Cancel",
            deletelStr: "Delete",
            doneStr: "Done",
            multiDragErrorStr: "Multiple File is not allowed.",
        ...
        var uploadLabel = $('<a>' + s.uploadStr + '</a>'),
            dragDropStrSpan = $('<i class="uk-icon-cloud-upload uk-icon-small uk-text-muted uk-margin-small-right"></i>');
        ...
        (function checkAjaxFormLoaded() {
            if($.fn.ajaxForm) {
                ...
                if(s.dragDrop) {
                    var dragDrop = $('<div class="' + s.dragDropContainerClass + '" style="margin-bottom: 10px;"></div>').width(s.dragdropWidth);
                    $(obj).append(dragDrop);
                    $(dragDrop).append(dragDropStrSpan);
                ...
                $(obj).append(obj.errorLog);
   				...
   				if(s.showQueueDiv)
		            ...
		            obj.container = $("<div class='uk-width-small-1-1'></div>").insertAfter($(obj));
	   ...
	   this.startUpload = function () {
        ...
        function setDragDropHandlers(obj, s, ddObj) {
            ...
            ddObj.on('drop', function (e) {
                ...
                if(!s.multiple && files.length > 1) {
                    if(s.showError) $("<div class='" + s.errorClass + "' data-uk-alert><a href='' class='uk-alert-close uk-close'></a>" + s.multiDragErrorStr + "</div>").appendTo(obj.errorLog);
                    return;
                }
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
                if(!isFileTypeAllowed(obj, s, files[i].name)) {
                    if(s.showError) $("<div class='" + s.errorClass + "' data-uk-alert><a href='' class='uk-alert-close uk-close'></a><b>" + files[i].name + "</b> " + s.extErrorStr + s.allowedTypes + "</div>").appendTo(obj.errorLog);
                    continue;
                }
                if(!s.allowDuplicates && isFileDuplicate(obj, files[i].name)) {
                    if(s.showError) $("<div class='" + s.errorClass + "' data-uk-alert><a href='' class='uk-alert-close uk-close'></a><b>" + files[i].name + "</b> " + s.duplicateErrorStr + "</div>").appendTo(obj.errorLog);
                    continue;
                }
                if(s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                    if(s.showError) $("<div class='" + s.errorClass + "' data-uk-alert><a href='' class='uk-alert-close uk-close'></a><b>" + files[i].name + "</b> " + s.sizeErrorStr + getSizeStr(s.maxFileSize) + "</div>").appendTo(
                        obj.errorLog);
                    continue;
                }
                if(s.maxFileCount != -1 && obj.selectedFiles >= s.maxFileCount) {
                    if(s.showError) $("<div class='" + s.errorClass + "' data-uk-alert><a href='' class='uk-alert-close uk-close'></a><b>" + files[i].name + "</b> " + s.maxFileCountErrorStr + s.maxFileCount + "</div>").appendTo(
                        obj.errorLog);
                    continue;
                }
                obj.selectedFiles++;
